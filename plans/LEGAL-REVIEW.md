...
**Diligence, not legal advice.** Items marked **COUNSEL** should be cleared by an
IP attorney before the DOI freeze. Method: four parallel reviews, each fetching
and quoting the *actual* upstream license/terms pages (not from memory). Verified
...
| **parquet** | ⚠️→OK (COUNSEL) | NYC TLC license is soft (generic NYC ToS, no explicit grant) but PII-clean (2024 zone-only schema: no medallion/driver/GPS). Only remaining NYC-TLC file after the csv/sqlite swap | attribution + rights note in NOTICE ✓ |
...
- **`exe` — FIXED with one counsel flag.** Kept the Hugo binary (member preserved)
  ...
  unmodified, source available upstream → documented + **flagged for counsel** to
  confirm MPL §3 is satisfied by upstream source availability.
...
1. **`mail` (Apache `users@httpd` mbox) — RISKY. Recommend DROP or replace.**
   ...
   RFCs under the IETF Trust license — no living-person PII). **COUNSEL.**
...
2. **`exe` (Hugo v0.162.1 binary) — RISKY. Recommend replace or bundle.**
   ...
   bundle (incl. MPL source pointer). **COUNSEL** on the MPL piece.
...
3. **`markup` (Silesia "xml") — RISKY. Recommend rebuild or get permission.**
   ...
   from S. Deorowicz / J. Cheney. **COUNSEL.**
...
- **Soft data licenses** (NYC TLC; previously GH Archive — now removed): where a
  ...
  + document and flag for **COUNSEL** comfort, rather than assert a license we
  weren't granted.
...
**2 items remain for counsel**:

1. **`parquet`** — the one remaining NYC-TLC file (soft license: open data, no
   explicit grant; PII-clean zone-only schema). Counsel to bless redistribution.
2. **`exe`** — Hugo binary, Apache-2.0, statically links an MPL-2.0 module;
   counsel to confirm MPL §3 is met by upstream source availability.

Either can be eliminated outright by swapping to an unambiguously permissive
source (offered). Nothing should be DOI-frozen until counsel clears these two.
